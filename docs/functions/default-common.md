...
| Chuẩn hóa email | `normalize_email` | Internet email normalization | Trim và lowercase **domain בלבד**, giữ nguyên local part vì semantics case có thể phụ thuộc provider. | ` Nguyen.VanAn@EXAMPLE.GOV.VN ` → `Nguyen.VanAn@example.gov.vn` | `SELECT normalize_email(email);` |

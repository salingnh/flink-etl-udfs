...
| Phân loại cấu trúc MST | `vn_classify_tax_id_structure` | Việt Nam, structural | Dùng nhãn trung tính mô tả đúng **hình thức** MST: `base_10` hoặc `extended_13`; không suy diễn loại hình pháp lý hay trạng thái người nộp thuế. | `0101234567-001` → `extended_13` | `SELECT vn_classify_tax_id_structure(tax_id);` | **Ưu tiên function này khi cần phân loại MST.** |
| Phân loại MST legacy | `vn_classify_tax_id` | Backward compatibility | API cũ trả `enterprise` / `dependent_unit`. Nhãn `enterprise` có thể diễn giải quá hẹp cho mọi MST 10 số, vì vậy chỉ giữ để không phá job cũ. | `0101234567-001` → `dependent_unit` | `SELECT vn_classify_tax_id(tax_id);` | **Không dùng cho pipeline mới; chuyển sang `vn_classify_tax_id_structure`.** |
...
| Chuẩn hóa mã số thuế Việt Nam | `vn_normalize_tax_id` | Việt Nam, structural | Chuẩn hóa về `10digits` hoặc `10digits-3digits`. Chỉ kiểm tra cấu trúc; xác minh người nộp thuế phải dùng registry/reference data có thẩm quyền. | `0101234567001` → `0101234567-001` | `SELECT vn_normalize_tax_id(tax_id);` | Giữ domain-specific. |

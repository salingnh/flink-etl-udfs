...
Sau cleanup `0.5.0`, `osint_*` chỉ còn semantics thật sự riêng cho observation/account handle. Các transform internet/security/code/finance dùng chung được chuyển sang pack generic tương ứng.

| OSINT category | Representative data | ETL / normalization | Open-source building blocks | Cách triển khai trong repo |
| --- | --- | --- | --- | --- |
| Username / account discovery | username, handle, platform, profile URL | normalize handle; platform/profile collection ở collector | Sherlock, Maigret | `osint_normalize_username`; platform/profile URL dùng raw metadata + `net_*` nếu cần |
| Social profile snapshot | UID, bio, avatar, observation time | observation ID, ISO time, evidence hash | Platform-specific collectors | `osint_build_observation_id`, `etl_normalize_iso_datetime`, `sha256_fingerprint` |
...
Các collector, sanctions matching, entity attribution, RDAP/DNS query, geocoding, social-platform API, crawler và credential acquisition không thuộc scalar UDF layer.

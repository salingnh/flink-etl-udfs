...
| --- | --- | --- | --- | --- |
| Username / account discovery | username, handle, platform, profile URL | normalize handle; profile URL enrichment qua collector/service | Sherlock, Maigret, internal collectors | `osint_normalize_username`; `enrich_extract_profile_url` cho synchronous REST extraction trên Flink 2.2.1 SQL Gateway; `net_*` cho URL/domain canonicalization |
...
Collector, sanctions matching, entity attribution, RDAP/DNS query, geocoding, crawler và credential acquisition nhìn chung không thuộc scalar UDF layer. `enrich_extract_profile_url` là ngoại lệ có kiểm soát cho lookup nhỏ trực tiếp trong Flink 2.2.1 SQL Gateway: synchronous, `deterministic=False`, có timeout rõ ràng và yêu cầu route/capacity được kiểm soát. Với tải lớn, vẫn ưu tiên Async I/O/lookup service ngoài scalar UDF.
